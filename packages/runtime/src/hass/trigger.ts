...
/**
 * Called once per @hass/trigger node (see the block definition itself in
 * blocks/hass-trigger.ts). In the distributed topology this runs directly in
 * the flow-host's main thread (flow-host/src/main.ts), exactly like
 * @core/scheduler's registerScheduler used to before it moved into a Worker
 * — @hass/trigger nodes don't get their own Worker at all (see
 * WorkerManager's own doc comment), since the flow's one real HA connection
 * (getHass()) lives in that thread, not in a per-node Worker. The
 * `subscribe` hook on the block definition also exists for Phase 1's
 * in-process demo, which has no Worker/flow-host split to begin with. Wires
 * a live, read-only DA subscription that calls `onChange` whenever the
