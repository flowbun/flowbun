...
describe("DistributedExecutor hass call timeouts", () => {
  test("callHassAction resolves normally when the coordinator replies before the timeout", async () => {
    const sent: FlowHostToCoordinator[] = [];
    const executor = new DistributedExecutor({
      flow: fakeFlow(fakeNode("@hass/action")),
      // biome-ignore lint/suspicious/noExplicitAny: not exercised by this test
      workerManager: {} as any as WorkerManager,
      send: (msg) => sent.push(msg),
      log: noopLog,
      hassCallTimeoutMs: TIMEOUT_MS,
    });

    const promise = executor.execute({
      nodeId: "n1",
      inputs: { call: { domain: "light", service: "turn_on" } },
      port: "call",
      traceId: "t1",
      seq: 1,
    });
    ...
    const call = sent[0];
    if (call?.type !== "hass.action.call")
      throw new Error("expected hass.action.call");
    executor.handleActionResult({
  ...
  test("callHassAction rejects with a timeout error if the coordinator never replies, and doesn't hang the caller", async () => {
    const executor = new DistributedExecutor({
      flow: fakeFlow(fakeNode("@hass/action")),
      // biome-ignore lint/suspicious/noExplicitAny: not exercised by this test
      workerManager: {} as any as WorkerManager,
      send: () => {}, // simulates the coordinator's own hass.call() hanging forever — no reply ever sent
      log: noopLog,
      hassCallTimeoutMs: TIMEOUT_MS,
    });
  ...
  test("a late hass.action.result arriving after the timeout is a safe no-op", async () => {
    const sent: FlowHostToCoordinator[] = [];
    const executor = new DistributedExecutor({
      flow: fakeFlow(fakeNode("@hass/action")),
      // biome-ignore lint/suspicious/noExplicitAny: not exercised by this test
  ...
  test("callHassRead resolves normally when the coordinator replies before the timeout", async () => {
    const sent: FlowHostToCoordinator[] = [];
    const executor = new DistributedExecutor({
      flow: fakeFlow(fakeNode("@hass/read", { entity: "sun.sun" })),
      // biome-ignore lint/suspicious/noExplicitAny: not exercised by this test
      workerManager: {} as any as WorkerManager,
      send: (msg) => sent.push(msg),
      log: noopLog,
    ...
    const call = sent[0];
    if (call?.type !== "hass.read.call")
      throw new Error("expected hass.read.call");
    executor.handleReadResult({
      type: "hass.read.result",
      requestId: call.requestId,
      ok: true,
      reading: { entity: "sun.sun", state: "above_horizon", attributes: {} },
    });

    await expect(promise).resolves.toEqual({
      result: { entity: "sun.sun", state: "above_horizon", attributes: {} },
    });
  });

  test("callHassRead rejects with a timeout error if the coordinator never replies", async () => {
    const executor = new DistributedExecutor({
      flow: fakeFlow(fakeNode("@hass/read", { entity: "sun.sun" })),
      // biome-ignore lint/suspicious/noExplicitAny: not exercised by this test
      workerManager: {} as any as WorkerManager,
